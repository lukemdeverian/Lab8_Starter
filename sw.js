...
self.addEventListener('fetch', function (event) {
  ...
  //            network response.
    ...
    caches.open(CACHE_NAME).then((currentCache) => {
      return currentCache.match(event.request).then((cachedResult) => {
        if (cachedResult) {
          return cachedResult;
        }

        return fetch(event.request).then((networkResult) => {
          currentCache.put(event.request, networkResult.clone());
